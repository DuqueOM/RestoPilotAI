...
class MenuOptimizer:
    ...
    async def analyze(
        ...
        category_summaries = self._generate_category_summaries(item_metrics, column_mapping)
        ...
        insights = await self._generate_ai_insights(item_optimizations, category_summaries)
        
        # Categorize items by action
        items_to_promote = [o.item_name for o in item_optimizations if o.action == OptimizationAction.PROMOTE]
        items_to_review = [o.item_name for o in item_optimizations if o.action in [OptimizationAction.REPOSITION, OptimizationAction.BUNDLE]]
        items_to_remove = [o.item_name for o in item_optimizations if o.action == OptimizationAction.REMOVE]
        ...
        price_adjustments = [
            {
                ...
                "change_pct": round((o.suggested_price - o.current_price) / o.current_price * 100, 1) if o.suggested_price else 0
            }
        ...
        thought_process = self._generate_thought_process(item_metrics, item_optimizations)
    ...
    def _calculate_item_metrics(self, df: pd.DataFrame, col_map: Dict) -> pd.DataFrame:
        ...
        # Group by item
        metrics = df.groupby(item_col).agg({
            qty_col: 'sum',
            rev_col: 'sum'
        }).reset_index()
        ...
        if cost_col and cost_col in df.columns:
            ...
            metrics['margin'] = (metrics['total_revenue'] - metrics['total_cost']) / metrics['total_revenue']
    ...
    def _merge_with_menu(self, metrics: pd.DataFrame, menu_items: List[Dict]) -> pd.DataFrame:
    ...
    def _add_bcg_categories(self, metrics: pd.DataFrame, bcg_results: Dict) -> pd.DataFrame:
        """Add BCG categories to metrics."""
        if 'items' not in bcg_results:
            return metrics
        
        bcg_map = {item['name']: item.get('category', 'UNKNOWN') for item in bcg_results['items']}
        metrics['bcg_category'] = metrics['item_name'].map(bcg_map)
        
        return metrics
    
    def _generate_item_optimizations(self, metrics: pd.DataFrame) -> List[ItemOptimization]:
        ...
        for _, row in metrics.iterrows():
            action, priority, reasoning, expected_impact, suggested_price = self._determine_action(
                row, rotation_high, rotation_low
            )
            ...
            opt = ItemOptimization(
                ...
                current_margin=round(row.get('margin', 0.6), 2) if 'margin' in row else None,
        ...
        optimizations.sort(key=lambda x: (priority_order[x.priority], -x.combined_score))
    ...
    def _generate_category_summaries(self, metrics: pd.DataFrame, col_map: Dict) -> List[CategoryOptimization]:
        ...
        for category in metrics['category'].dropna().unique():
            ...
            recs = []
            if avg_margin < 0.5:
                recs.append(f"Category margin ({avg_margin:.0%}) below target. Review pricing.")
            if avg_rotation < 0.4:
                recs.append("Low category rotation. Consider menu placement or promotion.")
            if len(cat_data) > 10:
                recs.append("Large category. Consider consolidating similar items.")
            
            summaries.append(CategoryOptimization(
                category=category,
    ...
    def _identify_quick_wins(self, optimizations: List[ItemOptimization]) -> List[Dict[str, Any]]:
        """Identify quick wins - high impact, easy implementation."""
        quick_wins = []
        
        # Price increases on high-volume items
        for opt in optimizations:
            if opt.action == OptimizationAction.INCREASE_PRICE and opt.priority in [PriorityLevel.CRITICAL, PriorityLevel.HIGH]:
                quick_wins.append({
                    "type": "price_increase",
                    "item": opt.item_name,
        ...
        # Items to remove
        removal_candidates = [opt for opt in optimizations if opt.action == OptimizationAction.REMOVE]
    ...
    def _calculate_revenue_opportunity(self, optimizations: List[ItemOptimization]) -> float:
    ...
    def _calculate_margin_opportunity(self, optimizations: List[ItemOptimization]) -> float:
        """Calculate potential margin improvement."""
        low_margin_items = [o for o in optimizations if o.current_margin and o.current_margin < 0.5]
    ...
    async def _generate_ai_insights(
        self, optimizations: List[ItemOptimization], categories: List[CategoryOptimization]
    ) -> List[str]:
        """Generate AI-powered insights using Gemini."""
        insights = []
        
        # Analyze patterns
        high_priority = [o for o in optimizations if o.priority in [PriorityLevel.CRITICAL, PriorityLevel.HIGH]]
        if high_priority:
            insights.append(f"🎯 {len(high_priority)} items require immediate attention for pricing optimization")
        ...
        if stars:
            insights.append(f"⭐ {len(stars)} Star items identified - protect these with strategic pricing")
        
        dogs = [o for o in optimizations if o.bcg_category == 'DOG']
        if dogs:
            insights.append(f"🐕 {len(dogs)} Dog items may be candidates for menu removal")
        
        # Price increase opportunities
        increase_items = [o for o in optimizations if o.action == OptimizationAction.INCREASE_PRICE]
        if increase_items:
            avg_increase = sum(
                (o.suggested_price - o.current_price) / o.current_price 
                for o in increase_items if o.suggested_price
            ) / len(increase_items) * 100
            insights.append(f"💰 Average recommended price increase: {avg_increase:.1f}%")
        
        return insights
    
    def _generate_thought_process(self, metrics: pd.DataFrame, optimizations: List[ItemOptimization]) -> str:
