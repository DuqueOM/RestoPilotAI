...
class OrchestratorAgent(GeminiBaseAgent):
    ...
    async def run_full_pipeline(
        ...
        **kwargs,
        ...
        if resume_from_checkpoint and state.checkpoints:
            last_success = next(
                (c for c in reversed(state.checkpoints) if c.success),
                None
            )
            if last_success:
        ...
        try:
            ...
            stages = [
                ...
                (PipelineStage.COMPETITOR_ANALYSIS, self._analyze_competitors, competitor_data),
                (PipelineStage.SENTIMENT_ANALYSIS, self._analyze_sentiment, review_data),
                (PipelineStage.SALES_PREDICTION, self._run_predictions, thinking_level),
                (PipelineStage.CAMPAIGN_GENERATION, self._generate_campaigns, thinking_level),
                (PipelineStage.EXECUTIVE_SUMMARY, self._generate_summary, thinking_level),
            ]
            
            # Add verification if enabled
            if auto_verify:
                stages.append((PipelineStage.VERIFICATION, self._run_verification, thinking_level))
    ...
    async def run_with_progress(
        ...
        **kwargs,
        ...
        yield ProgressUpdate(
            ...
            message="Pipeline completed" if "error" not in result else f"Error: {result['error']}",
            data=result,
        )
    ...
    async def _run_stage(
        ...
        try:
            ...
            duration_ms = int((time.time() - start_time) * 1000)
            self._save_checkpoint(state, success=False, error=str(e), duration_ms=duration_ms)
            raise
    
    async def _extract_menus(self, state: PipelineState, menu_images: List[bytes]) -> None:
    ...
    async def _analyze_dishes(self, state: PipelineState, dish_images: List[bytes]) -> None:
    ...
    async def _process_sales(self, state: PipelineState, sales_csv: str) -> None:
        ...
        for record in reader:
            state.sales_data.append({
                ...
                "units_sold": int(record.get("units_sold", record.get("quantity", 0)) or 0),
                "revenue": float(record.get("revenue", record.get("total", 0)) or 0),
                "had_promotion": str(record.get("had_promotion", "")).lower() == "true",
                "promotion_discount": float(record.get("promotion_discount", 0) or 0),
            })
    ...
    async def _run_bcg(self, state: PipelineState, thinking_level: ThinkingLevel) -> None:
    ...
    async def _run_predictions(self, state: PipelineState, thinking_level: ThinkingLevel) -> None:
    ...
    async def _generate_campaigns(self, state: PipelineState, thinking_level: ThinkingLevel) -> None:
    ...
    async def _generate_summary(self, state: PipelineState, thinking_level: ThinkingLevel) -> None:
    ...
    async def _run_verification(self, state: PipelineState, thinking_level: ThinkingLevel) -> None:
    ...
    def _build_final_response(self, state: PipelineState) -> Dict[str, Any]:
        ...
        return {
            ...
            "thought_signature": {
                ...
                "avg_confidence": (
                    sum(t.confidence for t in state.thought_traces) / len(state.thought_traces)
                    if state.thought_traces else 0
                ),
            },
    ...
    def get_session_status(self, session_id: str) -> Optional[Dict[str, Any]]:
        ...
        state = self.active_sessions.get(session_id) or self.completed_sessions.get(session_id)
        if not state:
            return None
